...
from .adapters.mock import MockDetector
...
def evaluate(directory: str, labels_path: str | None = None, detector_name: str = "mock") -> dict:
    ...
    settings = Settings(
        safety_detector=detector_name,
        vlm="off",
        mongodb_uri="mongodb://127.0.0.1:27017",
        device_token_secret="x" * 32,
    )
    detector = (
        MockDetector(settings=settings) if detector_name == "mock" else build_adapters(settings).detector
    )
    ...
    for path in sorted(root.iterdir()):
        ...
        predicted = {
            candidate.kind.removesuffix("_visible") for candidate in evaluate_rules(detections, [], settings)
        }
