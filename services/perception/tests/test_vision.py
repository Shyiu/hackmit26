...
def test_mock_vlm_is_honest_about_not_knowing():
    result = MockDescriptionVLM().describe(b"\xff\xd8\xff", (0.1, 0.1, 0.2, 0.2), "keys")
...
def test_openai_request_sends_bbox_schema_and_image_mime():
    ...
    result = adapter.describe(b"\xff\xd8\xffimage", (0.4, 0.3, 0.2, 0.2), "keys")
    ...
    assert "The item should be: keys" in body["messages"][1]["content"][0]["text"]
...
def test_openai_unknown_room_becomes_no_room():
    ...
    result = adapter.describe(b"\xff\xd8\xffimage", (0.0, 0.0, 1.0, 1.0), None)
    assert result.room is None
    client.close()


def test_openai_unverified_item_becomes_unknown():
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json={"choices": [{"message": {"content": payload(item_visible=False)}}]})
