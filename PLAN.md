...
A wearable camera that remembers where things are and notices when someone's about to forget something, for people living with dementia.
...
6. Recognize caregiver-enrolled faces, so the system knows who's with the wearer, and alert the caregiver immediately if the wearer is out of the approved area alone.
...
- Automatic emergency response. Lost alerts go to the caregiver, who decides whether to act; nothing in this system calls 911 or anyone else on its own.
...
flowchart LR
  ...
  V -- "detections, faces, annotated frames" --> W
  V -- "sightings, frame_observations" --> M[(MongoDB Atlas<br/>+ Vector Search)]
...
| Piece | Stack | Job |
|---|---|---|
| `apps/web` | Next.js App Router, TypeScript, Tailwind, shadcn/ui, MongoDB Node driver | The `/wear` chest page, the flat `/sim` fallback, the caregiver dashboard, the REST API, the `/api/ask` voice endpoint, the routine evaluator, and Web Push for lost alerts |
| `services/perception` | Python, FastAPI, pymongo, optional detector/face adapters | Takes frames, detects and tracks items, matches faces against enrolled people, writes sightings and frame observations |
...
These features moved from stretch goals to MVP scope. They share one property the rest of the perception pipeline doesn't have: each one can page a caregiver immediately, out of band from the dashboard's normal poll. That path reuses the notification/device-token plumbing built for caregiver messages, just with new `notifications.kind` values (`lost_alert`, `proactive_reminder`) and no caregiver keystroke in the loop.
...
This is for two things, and only two: giving the wearer's dashboard a "who was with you" record, and giving the lost logic below a way to tell "alone" from "with someone the family knows." It is not a general stranger-recognition system, and the "Not in v1" list above is explicit that unenrolled faces stay anonymous.
...
- **Live.** The wearer's phone posts camera frames to `POST /api/scan/live/frames`, and the app proxies them to a [splat-slam](../splat-slam) server, a separate Python service that runs SLAM and splat training and publishes a growing splat. A wearer's scene is the newest splat-slam scene named exactly `mg-<patientId>`; `GET /api/scan/live` finds or creates it. Pin positions are splat-slam world coordinates, unchanged.

How a live detection becomes a 3D point:

1. splat-slam answers a kept frame with its `name`, such as `s0/000012.jpg`.
2. The phone posts the detections on that frame to `POST /api/scan/observations`: per item, the bbox centre as normalised `u, v`. The pin now has an observation and `position: null`.
3. Once SLAM poses the frame, `cameras.json` lists a camera whose `file` is that same name.
4. The caregiver's browser builds a ray from that camera through `u, v`, raycasts the splat, and `PUT`s the hit to `/api/scan/pins` with the frame name. The write is refused with 409 if a newer observation has replaced that frame.

Until step 4 runs, the viewer draws the arrow along the view ray instead of at a point.

Run it locally:

```bash
cd ../splat-slam && uv run splat-slam serve --port 8420
# apps/web/.env.local: SPLAT_SLAM_URL=http://127.0.0.1:8420
pnpm scan:sim --frames <dir>     # posts a folder of JPEGs as if a phone were streaming
```

For a GPU, `uv run splat-slam cloud --serve` starts the server in a Daytona sandbox and prints a URL and a key; set them as `SPLAT_SLAM_URL` and `SPLAT_SLAM_KEY`. The Daytona wallet needs GPU credits. A deployed Vercel app needs a public `SPLAT_SLAM_URL`, so a local server only works with local `next dev`. Without `SPLAT_SLAM_URL`, `GET /api/scan/live` reports `configured: false` and the other live routes return 503; the static scene still works. With the URL set and the server down, `GET /api/scan/live` reports `reachable: false` and the others return 502.

splat-slam's live `cameras.json` carries no frame aspect, so the viewer assumes a 9:16 portrait feed when it builds a pin's ray. A landscape feed, such as a laptop on `/sim`, puts pins too close to the middle of the frame. The viewer already reads a per-camera `aspect` and `fov_y_deg` when present; splat-slam's `_write_cameras` in `live.py` has to write them.

The static assets came from sibling repos: `mesh.glb` is splat-mesh's `out/mesh.glb` and `splat.spz` is splat-lite's `out/smooth.spz`. They are copied in and regenerated by hand. Decisions are in [ADR 0005](docs/decisions/0005-room-scan-3d.md).

Status: not yet tested on a phone or against a Daytona sandbox.
...
// notifications: caregiver messages, reminders, and the two system-generated
...
// notifications are built on the client and never stored here.
{
  ...
  kind: "caregiver_message" | "reminder" | "proactive_reminder" | "lost_alert",
  text: "Lunch is at noon.",
  showAt, // reminders only
  status: "queued" | "shown" | "expired",
  shownAt,
  routineId // set on the kinds that reference a routine
}
...
- The perception safety MVP adds `frame_observations` and encrypted `people.faceEmbeddings`; frame observations are single-frame evidence records.
...
| `scanPins` | unique `{ patientId: 1, itemId: 1, sceneId: 1 }` | one 3D pin per item per scene, see "3D room scan" |
| `scanPins` | `{ patientId: 1, sceneId: 1 }` | every pin the viewer draws in one scene |
...
- **Alerts.** Lost/out-of-area alerts, newest first, each with an acknowledge action. These also arrive as a push notification with a distinct sound the moment they fire; this tab is the log, not the only way to see them.
- **Routines.** Add and edit the time-based and leaving-the-house reminders described under "Faces and routines," and see when each last fired.
- **Map.** The 3D room scan: the static demo room or the live splat, with an arrow at each item's last seen spot. See "3D room scan".
...
| `GET /api/scan/pins?sceneId=` or `?itemId=`, `PUT /api/scan/pins` | List 3D pins with the item name joined in. `PUT` sets a position (`manual` or `slam`); with `frame`, 409 when the stored observation has moved on |
| `POST /api/scan/observations` | The wearer's device reports where items sat on a kept frame: `sceneId`, `frame`, and up to 20 `{ itemId, u, v }` |
| `GET, POST /api/scan/live` | Live scan state: finds or creates the splat-slam scene `mg-<patientId>`. `POST { action: "reset" }` starts a fresh scene. `configured: false` without `SPLAT_SLAM_URL` |
| `POST /api/scan/live/frames` | One JPEG with `X-Session` and `X-Time` headers, proxied to splat-slam. Returns `{ sceneId, kept, reason, name }`. Device token or caregiver session |
| `GET /api/scan/live/scenes/:sceneId/cameras`, `.../splat/:file` | Passthrough of `cameras.json` (404 until the first map) and the immutable splat files (`v0003.spz`). Both check that the scene is the caller's `mg-<patientId>` |
...
| `GET /api/notifications`, `POST /api/notifications/:id/shown` | The device polls for the next queued caregiver message, due reminder, or fired routine, then marks it shown. Lost alerts also arrive this way as a fallback, but don't wait for the poll: see push, next |
| `POST /api/notifications` | The caregiver queues a message or a reminder by hand; the routine evaluator and the lost check queue the other two kinds automatically |
| `POST /api/push/subscribe` | Registers a caregiver or wearer device's Web Push subscription, so `lost_alert` notifications page immediately instead of waiting for the next poll |
...
# 3D room scan, optional. A splat-slam server; the key is sent as ?k=KEY when the server has one
SPLAT_SLAM_URL=
SPLAT_SLAM_KEY=
...
| M1b Faces | 1 to 8, parallel | Face enrollment on the dashboard, embedding + match in the perception service | An enrolled face is matched on a live frame; a  |
...
| M6 Optional extension | 12 to 18, only after core checks pass | Choose one: semantic questions, visual enrollment beyond faces, or a richer dashboard. Wake word, room enrollment beyond the door, and recording upload compete for this time | The chosen feature passes an evaluation without destabilizing M3 |
...
5. Have an enrolled teammate step into frame and show the live view label their face.
...
- More caregiver alert types, for example when the wallet hasn't been seen in two days.
